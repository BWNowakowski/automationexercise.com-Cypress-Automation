...
describe("automationexercise.com Test Suite", () => {
    ...
    it("Test Case 22: Add to cart from Recommended items", () => {
        cy.get("#recommended-item-carousel")
            .scrollIntoView()
            .should("be.visible")
            cy.get("#recommended-item-carousel > div > div.item.active > div:nth-child(1) > div > div > div > a").click()
            cy.get("div.modal-body > p.text-center > a[href='/view_cart']").click()
    ...
    it("Test Case 24: Download Invoice after purchase order", () => {
        ...
        cy.get("li[class='active']").should("have.text", "Shopping Cart")
        cy.get("a.btn.btn-default.check_out").click()
        cy.get("div.modal-body > p.text-center > a[href='/login']").click()
        cy.fixture("user").then( (data) => {
            const signup = new Register();
            signup.register(data.username, data.email)
            signup.inputFullDetails()
        ...
        const paymentDetails = new PaymentDetails()
        paymentDetails.paymentDetails()
        // cy.get("div#success_message > div.alert-success.alert").should("be.visible")
        // cy.get("div#success_message > div.alert-success.alert").should("have.text", "Your order has been placed successfully!")
    ...
    it("Test Case 25: Verify Scroll Up using 'Arrow' button and scroll down funcionality", () => {
        cy.scrollTo("bottom")
        cy.get("div.single-widget > h2").should("be.visible").should("have.text", "Subscription")
        cy.get("#scrollUp").click()
        cy.get("div.item > div.col-sm-6 > h2")
            .should("be.visible")
    ...
    it("Test Case 26: Verify Scroll Up without 'Arrow' button and scroll down funcionality", () => {
        cy.scrollTo("bottom")
        cy.get("div.single-widget > h2").should("be.visible").should("have.text", "Subscription")
        cy.scrollTo("top")
        cy.get("div.item > div.col-sm-6 > h2")
